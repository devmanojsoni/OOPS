...
class Rectangle {
    ...
    public: // make functions public
        ...
        // realized after setting l=0, and b=0 inside the parameter of the parameterised constructor
        Rectangle(){ // non-parameterized constructor
            length = 0;
            breadth = 0;
        }
        */
        // Let the constructor use the setter and getter functions
        Rectangle(int l=0, int b=0){ // parameterized constructor
            setLength(l);
            setBreadth(b);
        }
        // Copy Constructor
        Rectangle(Rectangle &rect){ // parameter is of type class Rectangle itself, passs by reference
            length = rect.length;
...
int main() {
    ...
    //Rectangle r(); // dec + default init
    Rectangle r(10,5); // dec + paramaterised init

    Rectangle r2(r); // itself the data-type passed
    int currentLength = r2.getLength();
    int currentBreadth = r2.getBreadth();
    cout<<currentLength<<endl;
    cout<<currentBreadth<<endl;
